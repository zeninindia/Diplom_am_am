...
class ApiPage:
    """
    Класс для взаимодействия с API Кинопоска.
    """

    def __init__(self, url: str | None) -> None:
        """
        Инициализация класса ApiPage.

        Атрибуты экземпляра:
        - self.url (str): базовый URL API;
        - self.key (str | None): API‑ключ из переменной окружения KEY;
        - self.headers (dict): заголовки для запросов, включая 'X-API-KEY'.
        """
    ...
    def search_movies1(self, film_name) -> tuple[dict, int]:
        ...
        (в текущей реализации не используется —
          вместо него берётся значение из переменной в .env FILM_NAME).
    ...
    def search_genres_name(self) -> requests.Response:
        ...
        with allure.step("Определение params: 'ручка' запроса"):
            params = {
                'field': 'genres.name'
            }
